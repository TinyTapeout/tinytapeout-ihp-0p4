...
This is a Programmable Reconfigurable Indexed State Machine (PRISM) that executes a Verilog coded Mealy state machine loaded via a runtime loadable configuration bitstream generated by a custom branch of Yosys.
PRISM includes it's own counter, shift register and compare sub-peripherals for controlling input/output data as well as timing operation.
